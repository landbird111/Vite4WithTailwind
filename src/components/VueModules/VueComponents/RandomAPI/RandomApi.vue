<template>
    ...
    <div class="flex justify-center items-center">
        ...
        <button @click.stop="getAPI" class="ml-4">
            <Icon size="24" tag="div" class="bg-indigo-600 hover:bg-indigo-700 transition ease-in-out">
                <RefreshFilled />
            </Icon>
        </button>
    </div>
    ...
    <div class="shadow-lg mt-8 max-w-lg block">
        <ul>
            <li v-for="item in getRandomData" :key="item.id.value"
                class="flex my-4 items-center border-b border-yellow-700 px-4 py-2">
                {{ item.name.title }} {{ item.name.first }} {{ item.name.last }}
                <img :src="item.picture.thumbnail" alt="avatar" class="ml-auto" />
            </li>
        </ul>
    </div>
</template>
...
//提供一個方法，用來取得隨機產生的資料
const getAPI = async () => {
    const res: IRandomApi = await axios.get(`https://randomuser.me/api/?results=${getRandomResult.value}`);
